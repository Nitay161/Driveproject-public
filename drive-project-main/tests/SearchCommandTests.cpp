...
TEST_F(SearchCommandTest, ExecuteSearchSingleMatch){
    ...
    searchCmd->execute(searchArgs.size(), searchArgs);

    // Restore cout
    std::cout.rdbuf(old);

    EXPECT_EQ(output.str(), "file1.txt\n");
}
...
TEST_F(SearchCommandTest, ExecuteSearchMultipleMatches){
    ...
    EXPECT_TRUE(result.find("a.txt") != std::string::npos);
    EXPECT_TRUE(result.find("c.txt") != std::string::npos);
    EXPECT_TRUE(result.find("b.txt") == std::string::npos);
}
