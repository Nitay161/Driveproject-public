...
TEST(AddInputTest, InterpretingAddInputCorrectly) {
    ...
    vector<string> args1 = {"add", "file1", "some text for f1"};
    EXPECT_EQ(add->interpret("add file1 some text for f1"), args1);

    vector<string> args2 = {"add", "file2", " some   other  text"};
    EXPECT_EQ(add->interpret("add file2  some   other  text"), args2);
}
...
TEST(AddInputTest, InvalidInputFormat) {
    ...
    EXPECT_THROW(add->interpret("add  file some text"), invalid_argument);
    // The input should contain text after the file.
    EXPECT_THROW(add->interpret("add file"), invalid_argument);
}
